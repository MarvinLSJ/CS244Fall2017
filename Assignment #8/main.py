...
def findRatio(IR, IRpeakindex, RED, REDpeakindex):
    ...
    minpointy = []
    minpointx = []
    for i in range(0, len(IRpeakindex)- 4):
...
data = np.genfromtxt("running.txt")
hrw = 0.75
fs = 50
size = data.shape[0]
print(size)
...
# IR = data['IR']
# RED = data['RED']
Time = np.arange(0, size*0.02, 0.02)
IR = data[:,0]
RED = data[:,1]
...
# print('diff', np.diff(IRpeaklist))
distance = np.mean(np.diff(IRpeaklist))
heartrate = 60 * 50 / distance

SPO2 = -45.060*Ratio* Ratio + 30.354 *Ratio + 94.845
print('SPO2',np.mean(SPO2))
